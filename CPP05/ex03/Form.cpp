...
/*   Updated: 2022/07/28 01:17:17 by aessakhi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Form::execute(const Bureaucrat &executor) const
{
	...
	{
		std::cerr << this->_name << " couldn't be executed. Reason: " << executor.getName(); 
		std::cerr << " grade is too low (" << this->_to_exec << " lowest grade required)" << std::endl;
		return;
	}
	catch (std::invalid_argument &e)
	{
		std::cerr << this->_name << " couldn't be executed. Reason: Form was not signed" << std::endl;
		return ;
	}
	this->execForm();
}

void	Form::execForm() const
{
